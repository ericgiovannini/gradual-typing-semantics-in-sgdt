...
\item In Section \ref{sec:concrete-term-model} we develop a denotational
  semantics in synthetic guarded domain theory for the \emph{terms} of the
  gradual lambda calculus. The model validates the equational theory, but it
  does not prove graduality as it does not model type or term precision. We use
  this to introduce some of our main technical tools: modeling recursive types
  in guarded type theory and modeling effects using call-by-push-value. After
  defining the model, we use the technique of globalization to extract a
  well-behaved big-step term semantics satisfying adequacy.
