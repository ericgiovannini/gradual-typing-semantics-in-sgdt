...
extended with additional cases to model effects besides error and stepping. % TODO cite guarded interaction trees
...
Additionally, while we have focused on a single semantics in this work, there are a
variety of other cast calculi that satisfy graduality but for which one or more
equational axioms fail to hold. \eric{Max, please add examples}
We expect that our development can be adapted to other cast semantics to prove
graduality and equational reasoning principles (to the extent that the
principles hold in those cast semantics).

We also plan to use our framework to investigate the performance of
gradually-typed languages. Although we have focused on proving extensional
properties in this work, the intensional nature of our semantics can be viewed
as a form of \emph{cost semantics}, where a runtime cost is incurred from
inspecting the dynamic type. This could be used to verify that cast
optimizations such as space efficient implementations
\cite{herman-tomb-flanagan-2010} are not only extensionally correct, but also
have lower quantitative cost.
