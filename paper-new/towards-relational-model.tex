...
write $c : A \rel A'$. The paradigmatic example of a poset relation is given by
the ordering $\ltdyn_A$. We call this the reflexive poset relation and
denote it by $r(A) : A \rel A$. This is a poset relation because the ordering
$\ltdyn_A$ is transitive.
...
error domain $\sem{M},\sem{M'} : U\li \sem{A}$, and we model the term precision
semantically as the relation holding between those denoted elements $\sem{M}
\binrel{(U\li\sem{c})} \sem{M'}$. Here $U\li\sem{c}$ is a relational lifting of
$U\circ\li$, which extends $\sem{c}$ to account for error and computational
steps.
%
