...
functions. The definition of square is as follows. Let $A_i, A_o,
A_i', A_o'$ be partially-ordered sets. Let $c_i : A_i \rel A_i'$ and
$c_o : A_o \rel A_o'$ be poset relations, and let $f : A_i \to A_o$
and $g : A_i' \to A_o'$ be monotone functions. We say that $f
\ltsq{c_i}{c_o} g$ if for all $x : A_i$ and $y : A_i'$ with $(x, y)
\in c_i$, we have $(f(x), g(y)) \in c_o$. We call this a \emph{square}
because we visualize this situation as follows:
%
...
category} as used in previous work \cite{newlicata}.
