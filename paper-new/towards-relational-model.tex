...
At this point we might try to weaken our initial attempt at
...
We call such a relation between $A$ and $A'$ a \emph{poset relation},
and we write $c : A \rel A'$ to mean $c$ is a poset relation downward
closed in $A$ and upward closed in A'. The paradigmatic example of a
poset relation is given by the ordering $\ltdyn_A$. We call this the
reflexive poset relation and denote it by $r(A) : A \rel A$. This is a
poset relation because the ordering $\ltdyn_A$ is transitive.
% The paradigmatic example of a poset relation is the ``reflexive'' relation,
% which is given by the ordering $\ltdyn_A$ and denoted by $r(A)$.
%
...
Poset relations are then used in the semantics of \emph{term}
precision. For closed programs $M \ltdyn M' : c$ where $c : A \ltdyn
A'$, the terms denote elements of the free error domain
$\sem{M},\sem{M'} : U\li \sem{A}$, and we model the term precision
semantically as the relation holding between those denoted elements
$\sem{M} \binrel{(U\li\sem{c})} \sem{M'}$. Here $U\li\sem{c}$ is a
relational lifting of $U\circ\li$, which extends $\sem{c}$ to account
for error and computational steps.
%
...
\ltsq{r(A_i)}{r(A_o)} f$ arising from the fact that $f$ is
monotone.
%
...
must be transitive, so we cannot entirely drop posets from our semantics.
...
reflexive, transitive and anti-symmetric given that the underlying relation $\ltdyn_A$
...
The definition is nearly identical, except that in the case when both sides are $\eta$ we
...
Observe that we can remedy this particular situation by
...
% composition is essential for modeling the axioms of gradual typing in a
...
\subsubsection{Modeling Term Precision}\label{sec:modeling-term-precision}
