...
%
% Given predomains $A_i$ and $A_o$ and morphisms $f, g : A_i \to A_o$, we define
% $f \bisimsq{A_i}{A_o} g$ to be the natural extension of bisimilarity to
% morphisms: given bisimilar inputs, the morphisms have bisimilar outputs.
...
%
We refer to the three injections as $e_\nat$, $e_\times$,
...
\begin{remark}
  ...
  % perturbation $\delta_{1,2}$ that is interpreted as the composition of the
...
\textbf{quasi-left-representable} by a predomain morphism $e_c : A \to A'$ if there
...
\textbf{quasi-right-representable} by an error domain morphism $p_d : B' \multimap B$ if
...
\begin{definition}\label{def:quasi-equivalent}
...
\begin{definition}
  ...
  and $g : A_i' \to A_o'$.  An \textbf{extensional square} $f
...
of the extended version \cite{gtt-sgdt-extended-version}).
%
...
% Interpretations of functions are different: one is arbitrary functions, the other is monotone functions that preserve bisimilarity
...
endomorphism $(\delta \circ \eta)^\dagger$, and the second sends $m_A \in M_A$ to
