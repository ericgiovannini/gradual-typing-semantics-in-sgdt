...
\begin{definition}
  ...
  \begin{enumerate}
     \item (Respects error): For all $y \in B'$, we have $(\mho_B, y) \in d$.\max{can we please use infix notation consistently for relations?}
...
\end{definition}
%
As with predomain relations, we can compose error domain
relations. However, because of a technical restriction\max{technical
  restriction?? isn't is simpler/correct to say that the composition
  of underlying relations is not an error domain relation?} involving
the interaction between propositional truncation and the later
modality, we cannot define the composition of error domain relations
as the composition of their underlying predomain relations. Instead,
we define composition of error domain relations $d$ on $B_1$ and $B_2$
and $d'$ on $B_2$ and $B_3$ inductively to be the least relation
containing $d$ and $d'$ that is downward closed under $\le_{B_1}$,
upward-closed under $\le_{B_3}$, respects error, and preserves
$\theta$.  Specifically, it is defined inductively by the following
rules:\max{what rules??}
% %
% \begin{mathpar}
...
c$ we have $(f(x), g(x')) \in Ud$. The fact that this relation preserves error\max{preserves error?}
...
\max{see above comment about reordering these concepts}
do not in general compose: in order to construct
...
The ordering $\le_D$ is given by:\max{this is kind of obvious IMO maybe it can go in the appendix}
%
\begin{align*}
    \inat(n) \le \inat(n') 
        &\iff n = n' \\
...
\bisim \id \}$. We claim\max{why we claim? Just say it's true!} that the three relations $\inat$, $\itimes$, and
...
We next claim\max{claim} that the relations $\inat$, $\itimes$, and $\iarr$ are
...
\ref{sec:term-interpretation}.\max{why is there a paragraph break here?}
...
up to a delay.\max{retraction property? has this ever come up anywhere in the paper before? Should it?}
...
\begin{definition}[quasi-order-equivalence]\label{def:quasi-order-equivalent}
  ...
  \delta^r_2 \in M_{A'}$ such that there is a square $\delta^l_1 \ltsq{c}{c'}
  \delta^r_1$ and a square $\delta^l_2 \ltsq{c'}{c} \delta^r_2$.
\end{definition}
Given computation objects $B$ and $B$, we make the analogous definition for
relations $d, d' : B \rel B'$ between the underlying error domains.
...
%
Unfortunately\max{it's not unfortunate}, this is actually not provable! Roughly speaking, the issue is
that this is a ``global''\max{what does ``global'' mean?} result, and it is not possible to prove such results
...
equality relation. We can define a ``global''\max{stop using scare quotes around ``global''} version of the lock-step error
...
relation on $\delay(\mathbb{N} + {\mho})$:\max{do these intermediate details matter for the main body of the paper?}
%
\begin{mathpar}
  \inferrule*[]
  { }
