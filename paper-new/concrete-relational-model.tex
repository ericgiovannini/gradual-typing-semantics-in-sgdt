...
\begin{itemize}
    \item A distinguished ``error'' element $\mho_B \in B$ such that $\mho \le_B x$ for all $x$
...
and $d'$ that is downward closed under $\le_{B_1}$, upward-closed under
$\le_{B_3}$, respects error, and preserves $\theta$. See Definition
...
morphisms: Given $f : A \to A'$ we define $\li f = (\eta \circ f)^\dagger : \li A \to \li A'$.\max{what is the point of this sentence?}
...
%
The definition of the ordering $\le_D$ is provided in the appendix for completeness.
