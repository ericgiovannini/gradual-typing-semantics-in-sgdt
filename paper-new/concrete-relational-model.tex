...
\tilde{x'}_t)$. Likewise for bisimilarity.
%
We also give a predomain structure to the natural numbers $\mathbb{N}$, where
both the ordering and the bisimilarity relation are equality.\max{why natural numbers here? I don't think we need it. check and come back}
...
A \emph{predomain relation} $c : A \rel A'$ is a monotone relation as
defined in the previous section (no condition is required for
bisimilarity). The ``reflexive'' predomain relation is written $r(A)$
and is given by the ordering $\ltdyn_A$. The relational composition of
two predomains is also monotone, and $r(A)$ is the identity for this
composition.
...
constructions as in Section~\ref{sec:tm-model}, but now enriched to
...
Section~\ref{sec:tm-model} changed only in that we interpret it as a
...
a type in our semantics. We define computation objects to similarly be
...
\end{definition}
Here $f' \bisimsq{A_i}{A_o} f$ is the natural extension of
bisimilarity to morphisms: given bisimilar inputs the functions have
bisimilar outputs.

\subsection{Constructing the denotational semantics}
...
\begin{theorem}
  ...
  \item For every type precision equivalence $c \equiv c'$ we have a
    quasi-order-equivalence of relations $\sem{c} \bisim \sem{c'}$.
...
semantics in Section~\ref{sec:tm-semantics}. The only difference is
...
\eric{May be able to move some of this to the appendix} For our value
type semantics, we already have an interpretation as predomains, we
describe how we enhance each of the constructions we use
$U,\li,\to,\times,D$ with a notion of syntactic perturbation.
...
verifications in the appendix (Definition
...
\subsubsection{Interpreting Type Precision Equivalence}

To model type precision equivalence we need to show the following quasi-equivalences:
%
\begin{lemma}
The following hold:
\begin{itemize}
...
\end{lemma}
\begin{proof}
  Details in the appendix (Lemma \ref{lem:quasi-order-equiv-functors}).
\end{proof}
...
%
The congruence rules of term precision are modeled easily: congruence
squares hold both for bisimilarity as well as error ordering, and so
by combination we also have these for extensional squares.
%
The type precision equivalence rule follows because $c \equiv c'$
implies quasi-equivalence, and that quasi-equivalence implies that
$\id \ltsqbisim{\sem{c}}{\sem{c'}} \id$.

Most crucially, we verify the existence of the squares for the cast rules. For UpL, the
