...
composition'' $d \odot d' : B \rel B''$ that is inductively generated from the
...
\begin{figure}
  \begin{mathpar}
    ...
    {x \mathbin{(d \relcomp d')} z}

    \inferrule
    {x' \ltdyn_{B_1} x \and x \mathbin{(d \relcomp d')} z}
    {x' \mathbin{(d \relcomp d')} z}

    \inferrule
    {x \mathbin{(d \relcomp d')} z \and z \ltdyn_{B_3} z'}
    {x \mathbin{(d \relcomp d')} z'}

    \inferrule
    { }
    {\mho_{B_1} \mathbin{(d \relcomp d')} z}

    \inferrule
    {\later_t[ \tilde{x}_t \mathbin{(d \relcomp d')} \tilde{z}_t ] }
    {\theta_{B_1}(\tilde{x}) \mathbin{(d \relcomp d')} \theta_{B_3}(\tilde{z}) }
  \end{mathpar}
