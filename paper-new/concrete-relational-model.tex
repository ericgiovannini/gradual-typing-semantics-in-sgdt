...
\subsubsection{Actions on Objects}
...
\subsubsection{Actions on Relations}
...
as
%
\( M_D \cong (M_{D \times D}) \times M_{U(D \to FD)}. \)
%
Unfolding the definitions, this is the same as
%
\( M_D \cong (M_D \times M_D) \times (\mathbb{N} \times M_D^{op} \times \mathbb{N} \times M_D). \)
%
We now explain how to interpret these perturbations as endomorphisms.
We define $i_D : M_D \to \{ f : D \to D \mid f \bisim \id \}$ below,
...
%
\begin{align*}
 i_D(p_{\text{times}}, p_{\text{fun}}) &= \lambda d.\text{case $d$ of}  \\
 &\alt \tnat(m) \mapsto \tnat(m) \\
    &\alt \ttimes(d_1, d_2) \mapsto {\ttimes(i_{D \times D}(p_\text{times})(d_1, d_2))} \\
    &\alt \tfun(\tilde{f}) \mapsto {\tfun(\lambda t. i_{U(D \to FD)}(\tilde{f}_t))}
\end{align*}
    % \item $\ptb_D(1)$
...
\to P_{U(D \arr FD)}$ by \( \pull_{\iarr}(p_{\text{times}}, p_{\text{fun}}) =
p_{\text{fun}}, \)
