...
satisfies graduality. To manage the complexity, we carry out the
construction in two phases, beginning with a relational model
consisting of partially-ordered sets with a notion of bisimilarity and
then adding requirements for a structured set of syntactic
perturbations on each type as well as the notion of
quasi-representability for relations that models the cast rules. We
then discuss the construction of the dynamic type and the refined
denotation of term precision that relates terms that can be
synchronized to be in lock-step. We conclude by demonstrating the
...
\end{itemize} %\max{shouldn't this be $\ltdyn$?}
\end{definition}
...
$A$.
%
When we refer to the order on a predomain $A$ in isolation we will denote it
$r(A)$, and to express that two elements are related in $r(A)$ we will
write $x \ltdyn_A y$.
...
\ltdyn_{\later A} \tilde{x'}$ iff $\later_t(\tilde{x}_t \ltdyn_A \tilde{x'}_t)$.
...
is a function between the underlying sets such that for all $x, x'$, if $x \ltdyn_A
x'$, then $f(x) \ltdyn_{A'} f(x')$, and if $x \bisim_A x'$, then $f(x) \bisim_{A'}
f(x')$.
\end{definition}

We also enhance our error domains with ordering and bisimilarity
relation, and we impose a condition relating the $\theta$ morphism to
the bisimilarity relation. More concretely:
%
\begin{definition}
...
\end{itemize}\max{Don't we also need $\forall x. \mho \le x$?}
\end{definition}
%
...
\begin{definition}
  ...
  \begin{enumerate}
     \item (Respects error): For all $y \in B'$, we have $\mho_B \binrel{d} y$. %\max{can we please use infix notation consistently for relations?}
...
\end{definition}
%
As with predomain relations, we can compose error domain relations. However, the
composition of the underlying predomain relations is not an error domain
relation.
% Can explain this further in the appendix
Instead, we define composition of error domain relations $d$ on $B_1$ and $B_2$
and $d'$ on $B_2$ and $B_3$ inductively to be the least relation containing $d$
and $d'$ that is downward closed under $\ltdyn_{B_1}$, upward-closed under
$\ltdyn_{B_3}$, respects error, and preserves $\theta$. See Definition
\ref{def:free-comp-ed-rel} in the appendix for the precise definition.
...
\begin{definition}[value and computation objects + morphisms]
  ...
  homomorphism of monoids $M_A \to \morbisimid{A} := \{ f : A \to A \mid f
  \bisim \id_A \}$.

  Likewise, a \textbf{computation object} consists of an error domain $B$, a
  monoid $M_B$ and a homomorphism of monoids $M_B \to \morbisimid{B}$.
...
In this section, we discuss the actions of the functors on value and computation
objects. Specifically, we define the actions of the functors on the syntactic
perturbations and their interpretation as semantic perturbations.
...
The fact that value and computation relations compose is proved in the appendix
(Definition \ref{def:value-computation-rel-comp}). The actions of the functors
$\li$, $U$, $\times$, and $\arr$ are also defined in the appendix (Definition
...
%
The definition of the ordering $\ltdyn_D$ is provided in the appendix for completeness.

% given by:\max{this is kind of obvious IMO maybe it can go in the appendix}
%

%
%
We define three relations involving $D$ as follows. We define $\inat :
\mathbb{N} \rel D$ by $(n, d) \in \inat$ iff $e_\nat(n) \ltdyn_D d$. We similarly
define $\itimes : D \times D \rel D$ by $((d_1, d_2), d) \in \itimes$ iff
$e_\times(d_1, d_2) \ltdyn_D d$, and we define $\text{inj}_\to : U(D \arr \li D)
\rel D$ by $(f, d) \in \iarr$ iff $e_\to(f) \ltdyn_D d$.
...
domains. It may seem as though we need to define the perturbations for $D$ by
guarded recursion, but in fact we define them via least-fixpoint in the category
of monoids:
%
...
\bisim \id \}$. Then it is straightforward to check that the three relations $\inat$, $\itimes$, and
...
% \max{Missing a subsection on how to interpret the syntax into the semantics, i.e., the actual graduality proof}
\subsection{Interpreting the Relational Axioms}

\eric{Check this}

We now turn to the interpretation of the type precision and term precision
axioms of the cast calculus (Section \ref{sec:GTLC}).

The congruence rules for term precision hold in the model, as all of
constructions are monotone and preserve weak bisimilarity.
%
The interpretation of the cast rules was discussed above.
%

It remains to verify the equivalence rules for type precision. For example,
consider the rule $(c_i \ra c_o)(c_i' \ra c_o')\equiv (c_ic_i' \ra c_oc_o')$. To
validate this rule in the model, we construct two squares $\id \ltsqbisim{(c_i
\ra c_o)(c_i' \ra c_o')}{(c_ic_i' \ra c_oc_o')} \id$ and $\id
...
that are synchronized in the intensional ordering, it will be sufficient to
construct two intensional squares between the relations, with perturbations on
both sides. This motivates the definition of \emph{quasi-order-equivalence}.
...
Then we have the following quasi-order-equivalences:
%
\begin{lemma}
The following hold:
\begin{itemize}
...
\end{lemma}
\begin{proof}
  See appendix (Lemma \ref{lem:quasi-order-equiv-functors}).
\end{proof}
...
Recall the big-step term semantics $- \Downarrow -$ defined in Section
\ref{sec:big-step-term-semantics}. The result we seek to show is the following:
...
We have already established graduality above, which states that syntactic term
precision implies that the denoted terms are related in the semantic
