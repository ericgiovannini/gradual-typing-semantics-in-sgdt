...
of gradual typing that we end up with will not support compositional reasoning.
However, by decomposing the denotation of term precision in the above manner, we
can employ compositional reasoning provided we work only with the
\emph{lock-step error ordering}. Thus, the tentative plan going forward will be
to carry out the proofs compositionally using the lock-step ordering. We will
then apply the closure under weak bisimilarity in the denotation of term
precision given above to handle the aspects involving stepping.
