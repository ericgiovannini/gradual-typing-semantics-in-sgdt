...
> it is the main point of contention IMO.

> I also think it's a bit rambling and could be made more concise.
...
from the complication that the relation that interprets term precision is not transitive
...
> trivialise the notions employed in the present work.
  ...
  We agree that there are subtleties with the model, and we appreciate this
  highly insightful comment for bringing this to our attention.
  We do not agree that these issues impact the definition of the
  /lock-step error ordering/, as that is already proposition-valued and no
  truncation is involved. On the other hand, this discussion does apply to our
  /weak bisimilarity/ relation, whose definition mentions termination of elements
  of the lift monad and requires truncation to be proposition-valued.
  If we used existentials in the places that mention termination, the relation would
  be trivial in the topos of trees. Additionally, the closure of the lock-step error
  ordering under weak bisimilarity has a similar issue if we use existentials instead
  of sigmas to quantify over the intermediate terms. That is, this relation is also trivial
  in the topos of trees. In order for our constructions to be portable to other models of
  guarded type theory, we therefore avoid the use of truncation unless we require that
  the definition be a proposition for other reasons.
