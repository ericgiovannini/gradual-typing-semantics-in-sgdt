...
perceive that the main criticisms of the paper are in the writing and
are confident we can address these in revisisions.
...
- Provide more explanation for the abstract model constructions in the
  body of the paper, particularly the construction that adds perturbations
- Clarify the key steps in the construction of an extensional model
- Provide more background on the axioms for gradual typing
...
setting of SGDT. The purpose of the extensional model is to provide
the categorical structure needed to interpret the terms and
inequational theory of gradual typing. 

However, the concrete
extensional model we construct in the paper suffers from the
complication that the relation that interprets term precision is not
transitive (i.e., we cannot compose commuting squares horizontally).
...
does not allow horizontal composition of squares.
Specifically, we can prove that the relation ≾ defined by

  lx ≾ ly = Σ (lx' : L℧ X). Σ (ly' : L℧ Y). lx ≈ lx' ⊑ ly' ≈ ly

is not transitive (where ≈ is weak bisimilarity and ⊑ is the lock-step error ordering).
It is with this concrete result in mind that we formulated the definition of an
arbitrary extensional model. Admittedly, this was more for the purpose of organization
and managing complexity, as the only concrete instantiation of the definition of
extensional model is the concrete one above.

This lack of horizontal composition is not actually a problem as far as interpreting
the rules needed for establishing graduality. This is because transitivity
was never required in the original formulations of the dynamic gradual guarantee
of Siek et al. Transitivity was added to the inequational theory in the work of
New and Licata because it holds in their domain-theoretic model.
The lack of transitivity does require our notion of extensional
...
Howeverm this process does not give us an idealized extensional model.

FIXED
> Do not ask a question in the response. State clearly that it does
> not give an idealized extensional model.

In short, the process of globalization does not get us a "global" solution to
...
our results in Guarded Cubical Agda. We remark that work on this formalization
is ongoing. We would also like to emphasize, both here
...
*ERIC: Yes, I believe this holds for the lock-step error ordering.
...
In the model our horizontal morphisms are prop-valued.

*MAX*: I don't understand this answer
*ERIC*: What I'm trying to say is that our error domain relations are
prop-valued which I thought means that they aren't proof relevant.
...
1. Our treatment was based on the Greatest HITs paper which uses the
   (∞,1)-topos of trees where this is not an issue, the question is of
   if the development could be made more "agnostic" which would make
   it more portable to other guarded type theories.
2. In the 1-topos of trees model, the existential quantifier is
   tricky: our definition of weak bisimilarity and our closure of
   lock-step error ordering under weak bisimilarity are trivial in the
   1-topos of trees. However the lock-step error ordering doesn't use
   an existential quantifier or propositional truncation, and so has a
...
3. We have studied this issue since the reviews and we are confident
   ...
   would not impact any of the results of the paper. The
   *globalizations* of weak bisimilarity and the closure of error
   ordering under weak bisimilarity are provably propositions even if
   the original notions are Sets. This allows us to remove dependence
   on some axioms used in Greatest HITs in the paper and our ongoing
   Agda formalization.
