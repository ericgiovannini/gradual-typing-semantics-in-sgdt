...
> trivialise the notions employed in the present work.
  ...
  of the lift monad. If we truncated here, the relation would be trivial in the
  topos of trees. Additionally, the closure of the lock-step error ordering under weak
  bisimilarity has a similar issue if we use existentials instead of sigmas to quantify
  over the intermediate terms. That is, this relation is also trivial in the topos of trees.
  In order for our constructions to be portable to other models of guarded type theory,
  we therefore avoid the use of truncation unless we require that the definition be
  a proposition for other reasons.
