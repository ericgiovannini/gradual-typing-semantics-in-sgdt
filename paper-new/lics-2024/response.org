...
and issues related to naming. We plan to correct these and make the
technical portions clearer.
...
This is a consequence of the fact that, as Reviewer 1 speculates, the weak
bisimilarity relation on the guarded lift monad is not transitive (this is provable
internally), and thus our concrete extensional model, which we obtain by closing
...
concrete one above. (And in fact, from a pedagocial standpoint, it may make *more*
sense to describe the concrete extensional model first.)
...
As Reviewer 1 remarks, however, we can consider what happens when we leave the
...
is the fact that the above isomorphism between the globalized lift monad and Capretta's
delay monad only holds for certain types. That is, for an arbitrary type X, it is not
necessarily the case that ∀k.L℧^k X is isomorphic to Delay(X + 1). In the setting of
Ticked Cubical Type Theory, this requires that X satisfy a technical condition called
/clock-irrelevance/. But this poses a problem for the type Dyn (defined by guarded
recursion as described in the paper). We do not believe that the type ∀k.Dyn^k is
clock-irrelevant. This would mean that we do not have the nice characterization
of the global lift of ∀k.Dyn^k, and hence we would not be able to show that the
bisimilarity relation on this type is transitive.

We are in the process of exploring these issues and trying to futher elucidate the
relationship between the notion of idealized extensional model and the notion of
extensional model that we have formulated in this work.
...
Reviewer 2 mentions that the paper is difficult to follow and feels
heavily compressed. We sympathize with this viewpoint and would like to
propose a few changes that we hope will improve the exposition.
Upon further review, we feel that one the main sources of confusion is the
material related to the notion of extensional model and why we need a
notion of extensional model that differs from the "idealized" extensional
model. This is discussed briefly in Section 4.2, but as Reviewer 1 points
out, this section can use improvement. More specifically, in that section
we state that transitive reasoning is not possible when reasoning up to
weak bisimilarity, and that this means that our notion of extensional model
cannot support horizontal composition of squares. We agree that this statement
is not well-motivated; to better appreciate the issue, it would help if we
had previously introduced the /concrete/ extensional model with the guarded
lift monad and demonstrated there that the closure of the lock-step error
ordering under weak bisimilarity is not transitive. Likewise, we state in
Section 4.2 that the representability property breaks down in the presence
of observable steps. This would be easier to appreciate if we presented
(simplified version of) the concrete intensional model and showed that one
of the simple representability properties does not hold. This would also
help to motivate the need for perturbations.
...
- Better motivate our notion of extensional model and the failure of the
  ...
  discussion in the specific concrete model
...
- Discuss the limitations of globalization as a means to defining an
  idealized extensional model
...
Yes, GTT for us stands for Gradual Type Theory, and we will make this clear
in the revision.
...
The main consequence of making horizontal composition a lax morphism is that rather than
requiring F(cc') = F(c)F(c'), we only require a square with F(c)F(c') on top and F(cc') on
the bottom and identities on the left and right. Likewise for the functor U.
...
> Sorry if it is mentioned earlier and I just missed it.
  ...
  This is essentially because weak bisimilarity on the guarded lift
  monad is not transitive, and so the ordering up-to weak bisimilarity
  is not transitive.
