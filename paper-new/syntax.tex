...
given by functorial actions \cite{new-licata18,new-licata-ahmed2019}.

\max{talk about errors and divergence}
...
\max{update this below to fit the formulation elsewhere}
...
$\equidyn$ to mean that each side is $\ltdyn$ the other, with
reflexivity precision derivations.
\begin{enumerate}
...
\item If $\sem{M}\ltdyn \sem{M'}$ then
  ...
  \item If $\sem{M'} \equidyn \sem{\mho}$ then $\sem{M} \equidyn \sem{\mho}$
  \item If $\sem{M'}\equidyn \sem{n}$ then $\sem{M} \equidyn \sem{n}$ or $\sem{M} \equidyn \sem{\mho}$
  \end{itemize}
\end{enumerate}
The first two conditions ensure that our notions of distinct result of
a program are all preserved by the semantics. The last intuitively
states that if $M$ has a non-erroring behavior, then $M'$ exhibits the
same behavior.
