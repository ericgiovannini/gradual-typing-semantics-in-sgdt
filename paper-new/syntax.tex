...
Our presentation of type precision is non-standard. In the usual
formulation, reflexivity and transitivity are admissible, and whenever
$A \ltdyn A'$, there is a unique precision derivation witnessing
this. 
%
% EXTENDED VERSION
% These usual rules are given in Section \ref{sec:appendix-gtlc-syntax} in the
% Appendix\max{extended version}.
%
% CONFERENCE VERSION
These usual rules are given in the appendix of the extended version of this
paper \cite{gtt-sgdt-extended-version}. 
%  
The reason for our presentation choice has to do with our
semantics, in which type precision derivations denote relations
between semantic elements of the types. In our semantics,
equivalent type precision derivations do not denote \emph{equal}
relations, but instead they denote relations that are only
\emph{quasi-equivalent}, i.e., if two terms are related by one then
...
ultimately our graduality proofs still apply to the standard
formulation.
...
%
We elide the congruence rules for every type constructor, e.g., if
...
$\Omega$ term with the help of casts:
%
\[ \Omega = (\lambda (x:\dyn). (\dnc{\iarr} x)\, x)\, (\upc{\iarr}(\lambda (x:\dyn). (\dnc{\iarr} x)\, x)). \]
